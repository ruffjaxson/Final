...
router.post('/', async (req, res) => {
  ...
  // malformed.
  if (!req.body.firstName || !req.body.lastName || !req.body.lightsaberColor || !req.body.username || !req.body.password)
    return res.status(400).send({
      message: "first name, last name, username, password, and most of all, LIGHTSABER COLOR are required"
    });
  ...
  try {
    ...
    if (existingUser)
      return res.status(403).send({
        message: "This isn't the userName you're looking for"
      });
